...
class CodeSearch(object):
    ...
    def _Retrieve(self, url):
        ...
        request = Request(
            url=url, headers={'User-Agent': self.user_agent_string})
    ...
    def SendRequestToServer(self, compound_request):
        ...
        url = '{host}/codesearch/json?{qs}'.format(
            host=self.codesearch_host, qs=qs)
        result = self._Retrieve(url)
        return CompoundResponse.FromJsonString(result)

    def GetAnnotationsForFile(self,
                              filename,
                              annotation_types=[
                                  AnnotationType(
                                      id=AnnotationTypeValue.XREF_SIGNATURE)
                              ]):
    ...
    def GetSignatureForLocation(self, filename, line, column):
        ...
        if result.return_code != 1:
            raise Exception('Request failed. Response=%s' %
                            (result.AsQueryString()))
    ...
    def GetSignatureForSymbol(self, filename, symbol):
        ...
        raise Exception("Can't determine signature for %s:%s" % (filename,
                                                                 symbol))
    ...
    def GetXrefsFor(self, signature, edge_filter):
        ...
        if not refs or not hasattr(refs.xref_search_response[0],
                                   'search_result'):
            return []
    ...
    def GetCallTargets(self, signature):
        ...
        for result in refs:
            for match in result.match:
                if hasattr(match, 'grok_modifiers') and hasattr(
                        match.grok_modifiers,
                        'virtual') and match.grok_modifiers.virtual:
                    candidates.extend(
                        self.GetOverridingDefinitions(match.signature))
        if not candidates:
